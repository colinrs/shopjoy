...
func (s *themeService) ListThemes(ctx context.Context) ([]*ThemeDTO, error) {
	themes, err := s.themeRepo.FindAll(ctx, s.db)
	if err != nil {
		return nil, err
	}

	shop, _ := s.shopRepo.FindByTenantID(ctx, s.db)
...
func (s *themeService) GetCurrentTheme(ctx context.Context) (*CurrentThemeDTO, error) {

...
> **CRITICAL: NEVER directly edit `internal/handler/routes.go`, `internal/types/types.go`, or other auto-generated files.** These files start with `// Code generated by goctl. DO NOT EDIT.` and any manual edit will be silently overwritten on the next `make api` regeneration. Always go through the `.api` → `make api` flow described below.
...
**⚠️ CRITICAL: Always define middleware in `.api` files, NEVER edit `routes.go` directly.**

go-zero generates `routes.go` from `.api` files via `make api`. Manually editing `routes.go` to add middleware will:

- Be silently overwritten on the next `make api` regeneration
- Cause confusing "where did my middleware go?" bugs after regeneration
- Break the `.api` → handler → logic → regen cycle that is the project's source of truth

**Correct workflow for adding middleware:**

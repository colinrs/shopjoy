...
CREATE TABLE IF NOT EXISTS `orders` (
    ...
    `original_amount` BIGINT NOT NULL DEFAULT 0 COMMENT '改价前原金额(分)',
    `adjust_amount` BIGINT NOT NULL DEFAULT 0 COMMENT '改价金额(分)',
    `adjust_reason` VARCHAR(200) NOT NULL DEFAULT '' COMMENT '改价原因',
    `adjusted_by` BIGINT NOT NULL DEFAULT 0 COMMENT '改价操作人ID',
    `adjusted_at` BIGINT DEFAULT NULL COMMENT '改价时间(Unix时间戳(秒))',
    ...
    `cancelled_at` BIGINT DEFAULT NULL COMMENT '取消时间(Unix时间戳(秒))',

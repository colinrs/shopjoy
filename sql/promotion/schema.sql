...
CREATE TABLE IF NOT EXISTS `promotions` (
    ...
    `start_at` BIGINT NOT NULL DEFAULT 0 COMMENT '开始时间(Unix时间戳(秒))',
    `end_at` BIGINT NOT NULL DEFAULT 0 COMMENT '结束时间(Unix时间戳(秒))',
...
CREATE TABLE IF NOT EXISTS `user_coupons` (
    ...
    `used_at` BIGINT DEFAULT NULL COMMENT '使用时间(Unix时间戳(秒))',
    `order_id` VARCHAR(64) DEFAULT '' COMMENT '订单ID',
    `received_at` BIGINT NOT NULL DEFAULT 0 COMMENT '领取时间(Unix时间戳(秒))',

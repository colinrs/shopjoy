...
|------|----------|
| 仅租户前缀 | `NIKE-3kF9xZmP` |
| 租户+商品前缀 | `NIKE-SHOE-3kF9xZmP` |
| 无前缀 | `3kF9xZmP`（系统默认） |

### 紧凑编码结构（9位 Base62）

```
[租户ID(4位)][小时时间戳(3位)][随机序列(2位)]
     ↓           ↓              ↓
   T001        7xK           fM
```
...
| 随机序列 | 2位 | `00` ~ `zz` | 3,844 种组合，配合时间戳防碰撞 |
...
func ParseSKU(code string) (*SKUInfo, error) {
    ...
    // 2. 解码租户ID（前4位）
    tenantID := decodeBase62(compactCode[0:4])

    // 3. 解码时间戳（中间3位）
    hourOffset := decodeBase62(compactCode[4:7])
    ...
    return &SKUInfo{
        ...
        TenantID:       tenantID,
        CreatedAt:      createdAt,
        RandomSequence: compactCode[7:9],
    }, nil
}
...
func ExtractTenantID(code string) (int64, error) {
    ...
    if len(compactCode) < 4 {
        return 0, ErrSKUParseFailed
    }

    return decodeBase62(compactCode[0:4]), nil
}
```
...
| 随机序列（2位） | 3,844 种组合，单租户单小时内理论最大 3,844 个 SKU |
| 数据库唯一约束 | `tenant_id + code` 唯一索引作为最终保障 |
| 冲突重试 | 检测到冲突时重新生成随机序列 |

**容量估算：**
- 单租户每小时：3,844 个（仅靠随机序列）
- 实际场景足够，冲突时可重试生成
